...
class __smart_sieve:
    ...
    @staticmethod
    def __extend_sieve() -> None:
        '''doubles the size of the sieve'''
        # double size of sieve's boolean values
        pre_len = len(__smart_sieve.__sieve)
        __smart_sieve.__sieve += [True] * pre_len
        # set values of known composites to false
        for prime in __smart_sieve.__prime_list:
            ...
            for multiple in range(start, len(__smart_sieve.__sieve), prime):
                __smart_sieve.__sieve[multiple] = False
    ...
    def __calculate_next_prime() -> None:
        ...
        possible = __smart_sieve.__prime_list[-1]
        ...
        while True:
            ...
            if possible >= len(__smart_sieve.__sieve):
                __smart_sieve.__extend_sieve()
            # check sieve boolean map to see if it is prime
            if __smart_sieve.__sieve[possible]:
                # prime found, add to known prime list and until boolean sieve
                __smart_sieve.__prime_list.append(possible)
                start = possible * possible
                end = len(__smart_sieve.__sieve)
    ...
    def is_prime(n: int) -> bool:
        ...
        while n > __smart_sieve.__prime_list[-1]:
            __smart_sieve.__calculate_next_prime()
        return __smart_sieve.__sieve[n]
    ...
    def nth_prime(n: int) -> int:
        ...
        while n > len(__smart_sieve.__prime_list):
            __smart_sieve.__calculate_next_prime()
        return __smart_sieve.__prime_list[n - 1]

    @staticmethod
    def last_known_prime() -> int:
        '''exposes last known prime, useful for optimizations'''
        return __smart_sieve.__prime_list[-1]
...
def prime_generator() -> Generator[int, None, None]:
    ...
    n = 1
    while True:
        yield __smart_sieve.nth_prime(n)
        n += 1
...
def is_prime(n: int) -> bool:
    ...
    if n <= __smart_sieve.last_known_prime():
        return __smart_sieve.is_prime(n)
...
def nth_prime(n: int) -> int:
    ...
    return __smart_sieve.nth_prime(n)
